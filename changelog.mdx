---
title: 'Changelog'
description: 'The latest updates, new features, improvements, and fixes shipped to AutoSend.'
...
<Frame>
  ![Campaign Insights card comparing a campaign with previous
  campaigns](/images/changelog/campaign-insights.png)
</Frame>

The Analytics tab of every sent campaign now has a **Campaign Insights** card that shows how the campaign performed compared with your recent campaigns.

- **Compared with your last campaigns** - Open rate, click rate, bounce rate, unsubscribes, and complaints are compared against the average of up to 5 earlier campaigns sent from the same project. With clear up-down signals, now it is easy for you to understand how your new campaign performed.
- **Completion time** - See how long the campaign took to reach all its recipients.
- **Final numbers include late activity** - 10 days after a campaign is sent, AutoSend saves its final numbers, so opens and clicks that come in later still count. For gradual send campaigns, the 10 days start after the last batch.

Insights appear once you've sent at least one earlier campaign to compare against.
...
<Frame>
  ![Opened and True Open rates in campaign
  analytics](/images/changelog/true-open.png)
</Frame>

Campaigns have been rebuilt from the ground up, from creating a campaign to reviewing how it performed.

#### Three tabs for sent campaigns

Sent campaigns now open in three tabs. Each tab has its own link, so you can share a direct view with your team.

- **Analytics** - See how your campaign performed, including a new **True Open** rate that leaves out automatic opens from Apple Mail Privacy Protection, Gmail's image proxy, and security scanners. Open and click rates are now calculated from delivered emails instead of sent emails, so they're more accurate.
- **Email Activity** - See every recipient with their status, opens, and clicks. Search by email, filter by status (including **Unsubscribed**), sort the list, and export it to work with outside AutoSend.
- **Details** - A read-only view of the template, campaign details, and tracking settings used for the send.

</Update>

<Update label="July 28, 2026">

## Fallback Values for Email Variables

Variables can now have a fallback value for contacts whose field is empty. For example, write `Hi {{first_name | "there"}}` and a contact without a first name sees "Hi there" instead of "Hi ".

#### Other changes:

1. **Segment filters for empty dates** - Date properties in segments now support **is empty** and **is not empty**, so you can, for example, find contacts who have never opened an email.
2. Your email sending limit now shows the exact time it resets, not just the date.
3. Resuming a regular (non-gradual) campaign now asks for confirmation first.
4. Your sending quota is no longer counted twice when a campaign is resumed or stopped.
5. Text colors in lists now display correctly in the email builder.

</Update>

<Update label="July 24, 2026">

## Know Where Every Contact Came From

Every contact now has a **Source** property that shows how it was added: manually, through the API, by CSV import, through MCP, from a SendGrid or Resend migration, or from an integration like Tally Forms. Use it to filter contacts or build segments.

## See When a Contact Last Opened Your Email

Contacts also have a new **Email Last Opened At** property. It only counts real opens, not automatic ones from privacy features and security scanners, so you can build segments of people who actually read your emails.

#### Other changes:

1. Date fields on a contact, like Email Last Opened At, now show the time as well as the date.
2. Email activity now shows the sender's email address when no sender name is set, instead of "undefined".
3. **MCP:** `list_templates` now supports search, filtering by template type, and pagination, replacing the separate `search_templates` tool.
4. **Invoices for credit top-ups** - Buying email credits now generates an invoice you can download from Billing.
...
<Frame>
  ![Tally Forms integration](/images/changelog/changelog-tally-integration.jpg)
</Frame>
...
## Rename Lists and Segments

You can now edit the name of any list or segment after creating it.

## Many Quality of Life Improvements

1. **Image library in the email builder** - Open your image library straight from the builder header in campaigns, templates, and automations. You can also copy any image's URL from its menu in one click.
2. **Filter email activity by automation** - Email Activity filters now include **Automation**, next to Template and Campaign, so you can see every email a specific automation has sent.
3. **Clearing contact fields works as expected** - Emptying a field on a contact now actually clears it. The API also accepts `null` for optional fields when creating or updating contacts.
4. **HTML size limit** - Email HTML is now capped at 4MB. Larger emails are rejected with a clear error instead of failing silently.
5. **Inbound attachment downloads** - Pass `download=true` when fetching an inbound attachment to get the file directly, instead of a download URL.
6. **Clearer Inbound API errors** - The Inbound Email API now returns more specific error messages with suggestions on how to fix them.
7. Scheduled campaigns now show correctly in campaign counts and lists.

</Update>

<Update label="July 7, 2026">

## Gradual Send, Made it better

Gradual send campaigns are now easier to control and much easier to follow.

- **Pause any time** - Pause a gradual campaign even before it starts sending, while it's still scheduled.
- **Resume with a preview** - Resuming now shows a confirmation with the new start date, time, and increase strategy before anything goes out.
- **See what's coming** - While a campaign is paused, the chart shows the remaining batches, contacts, and expected finish date based on your resume choice.
- **Full pause history** - Every pause and resume is recorded, with the reason shown on the chart (for example, "Manually paused" or "Daily limit reached").
- **Next batch time** - The campaign header shows how many contacts are done and when the next batch will go out.
- **Always visible** - Scheduled and paused campaigns now stay at the top of your campaign list, no matter how old they are.
...
<Frame>
  ![Improved Testing Email Experience](/images/changelog/test-email.png)
</Frame>

## Faster Test Emails and Contact Properties

#### Improved Testing Email Experience

- **Send to myself:** Testing an email no longer means typing your address every time. Pick **Send to myself** from the test email dropdown and the test goes straight to your account inbox.

- **Copy as cURL:** Happy with how a test looks? Click **Copy as cURL** to get a ready-to-run command for that exact send, payload included. Run it in your terminal or drop it into your codebase.

Together, these close the loop between designing an email and sending it from code.

#### Custom Fields are now Contact Properties

Custom Fields are now called **Contact Properties** across AutoSend. The new name matches how you actually use them.

_Please note:_ This is a rename only. Everything you've already set up keeps working. The API still accepts the old Custom Fields naming, so you can switch to Contact Properties whenever it suits you.

Read the <a href={APP_PATHS.contactsContactProperties}>Contact Properties docs</a> and the <a href={APP_PATHS.createContactPropertyApiRef}>API reference</a> to learn more.
...
<Frame>
  ![Official Convex component](/images/changelog/inbound-email-api.jpg)
</Frame>
...
Read <a href={APP_PATHS.inboundIntroduction}>how to setup and use the Inbound Email API</a> and browse the <a href={APP_PATHS.listInboundMessagesApi}>API reference</a> to learn more.

</Update>

<Update label="June 22, 2026">

## Encrypted Send Requests

For teams that need an extra layer of security, AutoSend now supports encrypted email-send requests. Fetch a public key and send an encrypted payload to the send endpoint, so sensitive email content is protected in transit beyond standard HTTPS.

Read the <a href={APP_PATHS.encryptedPayloads}>encrypted payloads guide</a> to learn more.

#### Other changes:

1. For gradual send, you can now change the send time even while a campaign is paused.
2. New sending domains now warm up on their own. AutoSend automatically raises your daily send limit a little each day to build a healthy reputation, with a clear banner showing your current limit. Read the <a href={APP_PATHS.domainWarmup}>domain warmup guide</a> for details.
3. Contacts can now store a mobile number as a default field.
4. Attachments and webhook events now carry stable, consistent identifiers with better validation, making it easier to reliably match events and files in your own systems.
5. Error messages across the API (campaigns, webhooks, events, projects, suppression groups, and more) were rewritten to be clearer and more helpful.

</Update>

<Update label="June 19, 2026">

## More Flexible Automations and Builder Margins

#### Wait steps on any branch

Delays used to live only on the main flow. Now you can add a wait step to any branch, so each path moves at its own pace. One branch can wait three days before a follow-up while another continues right away.

#### Property triggers now catch new contacts

Automations using the **Contact property matches with** trigger now also fire when a contact is created with a matching value, not only when that value changes later. New contacts enter the right journey from day one.

Read the <a href={APP_PATHS.automations}>Email Automations documentation</a> to learn more.

#### Margin controls in the email builder

Blocks in the email builder now support margins, so you can control the space around content directly instead of stacking padding or spacer blocks.

- Set margins per block for precise spacing between elements
- Build cleaner, more consistent layouts without workaround blocks

#### Fixes

- Icon colors now render correctly in the builder
- Preview spacing now matches what recipients see in their inbox

</Update>

<Update label="June 15, 2026">

## Resend Migration and Smaller Fixes

Switching from Resend? Use our migration API with your Resend API key to bring your contacts, templates, and suppression lists straight into AutoSend. Follow the <a href={APP_PATHS.migrationResend}>step-by-step migration guide</a>, so you don't have to rebuild anything by hand.

This release also cleans up delete API responses and fixes a handful of rough edges:

1. The campaign reply-to address now saves correctly
2. Polished email builder to remove few glitches.

</Update>

<Update label="June 12, 2026">

## Automations for Imported Contacts

When you import contacts from a CSV file, you can now drop them straight into an automation. AutoSend runs validation checks first, so only valid, sendable contacts get enrolled. This turns a plain import into the start of a live email sequence.

## Webhook Management via API

You can now create and manage your webhooks straight from the public API, alongside a fix that keeps scheduled gradual-send campaigns from showing a false "quota reached" warning or skipping paused days.

#### Other Changes:

1. Delete Your Account - You can now delete account right from the app, giving you full control over your data.
2. Cleaner Template Copying - Copying content from a template preview now gives you clean, readable text instead of raw HTML code, ready to paste anywhere.
...
AutoSend now works smoothly on your phone. We rebuilt the dashboard, email activity, campaigns, contacts, senders, events, account settings, automations, and webhook pages to look and work well on small screens, so you can check on your sending from anywhere.

#### Other changes:

1. Campaign redesign - Campaigns get a refreshed look and a clearer layout, making it easier to see what matters at a glance. Small touches include a refresh button on each campaign card and dedicated email activity for each campaign.
2. Copying content from a template preview now gives you clean, readable text instead of raw HTML code, ready to paste anywhere.
3. The sidebar now clearly flags any domain that isn't verified yet, so nothing slips through.
...
Read the <a href={APP_PATHS.accountBilling}>Billing</a> and <a href={APP_PATHS.accountUsage}>Usage</a> docs to learn more.
...
Read the <a href={APP_PATHS.smtpIntroduction}>SMTP quickstart</a> to learn more.
...
<Frame>
  ![Official Convex component](/images/changelog/autosend-convex-component.jpg)
</Frame>
...
Read the <a href={APP_PATHS.convexGuide}>Convex integration guide</a> for setup, sending variants, status tracking, and configuration.
...
<Frame>
  ![Event-triggered automations and EU
  region](/images/changelog/event-triggered-automation.png)
</Frame>
...
Read the <a href={APP_PATHS.events}>Events in Automations documentation</a> to learn more, and check the <a href={APP_PATHS.sendEventApi}>Send Event API reference</a> to start firing events from your backend.
...
Read the <a href={APP_PATHS.domainConfiguration}>Domain documentation</a> to learn more.
...
<Frame>
  ![RBAC and Projects API](/images/changelog/automation-branching.jpeg)
</Frame>
...
<Frame>![RBAC and Projects API](/images/changelog/rbac-changelog.jpg)</Frame>
...
<Frame>
  ![API Reference Endpoints](/images/changelog/new-api-endpoints.gif)
</Frame>
