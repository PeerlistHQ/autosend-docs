...
<Info>
	Building a multi-tenant system and need more than 10 projects? Reach out to{' '}
...
<Accordion title='Do all my projects share the same email sending quota?'>
	...
	plan, the total sends are shared between your 3 projects - there's no per-project quota split.
</Accordion>
...
<Accordion title='Can I delete a project?'>
  ...
  <Accordion title="I'm building a multi-tenant product. Is there a limit to how many projects I can have?">
    The Scale plan supports up to 10 projects. If you're building a product where each of your customers needs their own isolated project, contact <a href={`mailto:${SUPPORT_EMAIL}`}>{SUPPORT_EMAIL}</a> - we support custom project limits for multi-tenant use cases.
  </Accordion>
</AccordionGroup>
