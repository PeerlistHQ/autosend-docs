...
Triggered when an inbound email is received on one of your project's receiving domains. Use the message ID in the payload to fetch the full structured message via the [Get Message](/api-reference/inbound-emails/get-message) endpoint.
...
```json
{
  ...
  "data": {
    "messageId": "64f1a2b3c4d5e6f7g8h9i0j1",
    "from": {
...
- `messageId` - Unique identifier for the inbound message
...
- `domain` - The receiving domain the message was sent to
- `hasAttachments` - Boolean indicating whether the message has attachments
- `threadId` - Thread identifier for grouping related messages (optional)
...
<Columns cols={2}>
  <Card title="Introduction" icon="/icons/introduction.svg" href={APP_PATHS.webhookIntroduction}>
...
<Card
  ...
  icon='/icons/retries-refresh.svg'
...
<Card
  ...
  icon='/icons/verify-requests.svg'
  href={APP_PATHS.webhookVerifyRequests}
>
  Security and signature verification
</Card>

  <Card title="Webhooks" icon="/icons/webhook.svg" href={AUTOSEND_PATHS.webhooks}>
