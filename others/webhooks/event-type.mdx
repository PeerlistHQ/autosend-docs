...
**Note:** The message body (text/HTML) and full headers are not included in the webhook payload. To access those, call `GET /v1/inbound/messages/{inboundEmailId}` using the `inboundEmailId` from this event as the path parameter. The `messageId` field in the Get Message response is the RFC 5322 email header (e.g. `<abc@mail.gmail.com>`) - a different value entirely, not a database ID.
