...
async function getWebhookSecret() {
  const response = await fetch(`https://api.autosend.io/api/v1/webhooks/${WEBHOOK_ID}/reveal`, {
    method: 'GET',
    headers: {
...
def get_webhook_secret():
    url = f'https://api.autosend.io/api/v1/webhooks/{WEBHOOK_ID}/reveal'
    headers = {
    ...
    data = response.json()
    print('Webhook secret:', data['data']['secret'])
    return data['data']['secret']
...
$ch = curl_init("https://api.autosend.io/api/v1/webhooks/{$webhookId}/reveal");
...
echo 'Webhook secret: ' . $data['data']['secret'] . PHP_EOL;
?>
```
...
func main() {
    ...
    url := fmt.Sprintf("https://api.autosend.io/api/v1/webhooks/%s/reveal", webhookId)
...
uri = URI("https://api.autosend.io/api/v1/webhooks/#{webhook_id}/reveal")
...
puts "Webhook secret: #{data['data']['secret']}"
```
