---
title: 'API Errors'
description: 'Comprehensive guide to API errors and HTTP status codes'
---
...
| `503` | Service Unavailable | Temporary service outage | Maintenance or temporary unavailability |
...
3. Monitor rate limit headers to prevent exceeding limits
...
2. Check the AutoSend status page
...
if (response.status === 503) {
  // Service unavailable - wait longer before retrying
...
If you send the same idempotency key within 24 hours, you'll receive the same response as the original request without processing a duplicate operation.
