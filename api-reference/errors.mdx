...
2. Wait for the `retryAfter` seconds before retrying
3. Reduce the frequency of your API requests
...
if (response.status === 429) {
  const error = await response.json();
  const waitTime = error.retryAfter ? error.retryAfter * 1000 : 30000; // Default 30 seconds
