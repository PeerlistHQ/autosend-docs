...
description: 'Abort a running campaign to immediately stop all pending email sends using the AutoSend API.'
sidebarTitle: 'Abort Campaign'
openapi: 'POST /campaigns/{campaignId}/abort'
---
...
curl_setopt_array($curl, [
  CURLOPT_URL => 'https://api.autosend.com/v1/campaigns/60d5ec49f1b2c72d9c8b4567/abort',
...
### Response
<span className='text-sm'>Returns the updated campaign object with status set to `aborted`. Aborting a campaign immediately stops all pending sends. Emails already sent are not recalled.</span>
