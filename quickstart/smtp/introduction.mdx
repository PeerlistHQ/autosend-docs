...
<Frame>![Supabase Step 1](/images/smtp/smtp-screenshot-1.png)</Frame>
...
<Note>The API key secret will be generated with the flow of setting up the SMTP.</Note>
...
<Warning>
	Always use TLS/SSL encryption when connecting to SMTP. Never send credentials over an unencrypted
	connection.
</Warning>
...
<AccordionGroup>
	<Accordion title='Can I see emails sent via SMTP in my dashboard?'>
		Yes! All emails sent through SMTP appear in your AutoSend Dashboard under the{' '}
		<a href={AUTOSEND_PATHS.emailActivity} target='_blank' rel='noopener noreferrer'>
			**Email Activity**
		</a>{' '}
		section. You can view delivery status, opens, clicks, and bounces just like API-sent emails.
	</Accordion>
	<Accordion title='What are the rate limits for SMTP?'>
		...
		and API. Check your{' '}
		<a href={AUTOSEND_PATHS.dashboard} target='_blank' rel='noopener noreferrer'>
			Dashboard
		</a>{' '}
	...
	<Accordion title='Do webhooks work with SMTP emails?'>
		Yes! Webhooks are triggered for all events (delivery, bounce, open, click, etc.) regardless of
		whether the email was sent via SMTP or the API.
	</Accordion>
	<Accordion title='Can I use multiple sender addresses?'>
		...
		address in the "From" field. Make sure all sender addresses are properly configured in your
		domain settings.
	</Accordion>
</AccordionGroup>
